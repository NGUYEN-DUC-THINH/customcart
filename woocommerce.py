...
class LeCartWoocommerce(LeCartWordpress):
	...
	def category_import(self, convert, category, categories_ext):
		...
		id_category = self.import_category_data_connector(self.create_insert_query_connector('terms', category_data), True)
		...
		term_x_id = self.import_category_data_connector(self.create_insert_query_connector('term_taxonomy', term_x_data), True)
	...
	def product_import(self, convert, product, products_ext):
		...
		product_data = {
			...
			'post_excerpt' :'' ,
			'comment_status' :'publish' if convert['status']  else 'draft',
			'ping_status' : 'open' if convert['status']  else 'close',
		...
		id_product = self.import_category_data_connector(self.create_insert_query_connector('posts', product_data), True)
		self.insert_map(self.TYPE_CATEGORY, convert['id'], id_product, convert['code'])
		queries = {
			'type': 'update',
		...
		stock_st = 'instock' if convert['is_in_stock'] else 'outofstok'
		metakey = ['_edit_last','_sku','_regular_price','_tax_status','_tax_class','_manage_stock','_backorders','_sold_individually','_weight','_length','_width','_height','_stock_status','_product_version','_price','_stock']
		metaval = ['1',convert['sku'],convert['price'],'taxable','','yes','no','no',convert['weight'],convert['length'],convert['width'],convert['height'],stock_st,'4.5.2',convert['price'],convert['qty']]
		# for i in term_meta:
    	# 		category_term_meta = {
		# 		'term_id': id_category,
		# 		'meta_key': term,
		...
		for i in range(len(metakey)):
			post_meta = {
				'post_id': id_product,
				'meta_key': metakey[i],
				'meta_value': metaval[i]
			}
			self.import_product_data_connector(self.create_insert_query_connector('postmeta', post_meta), True, convert['id'])
	...
	def customer_import(self, convert, customer, customers_ext):
		# self.log(convert,'ct_convert')
		display_name = convert['first_name'] + convert['last_name']
		user = {
			'user_login': convert['first_name'] + convert['last_name'],

...
class LeCartCustom(LeBasecart):
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['name'] = html.unescape(product['products_model'])
		...
		# self.log(product_description,'product_description')
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		# customer_data['username'] = customer['']
	...
	def convert_order_export(self, order, orders_ext):
		...
		# order_data['tax']['amount'] = get_value_by_key_in_dict(orders_ext['data']['order_items'], 'products_tax', 0.0000)
		...
		bill_fname,bill_lname = order['billing_name'].split(' ',1)
		order_billing['first_name'] = bill_fname
		order_billing['last_name'] = bill_lname
		...
		#self.log(billing_country,'billing_country')
		if billing_country:
		...
		billing_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'zone_code', order[
			'billing_state'])
		#self.log(billing_state,'billing_state')
		if billing_state:
		...
		fname,lname = order['delivery_name'].split(' ',1)
		order_delivery['first_name'] = fname
		order_delivery['last_name'] = lname 
